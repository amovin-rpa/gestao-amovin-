...
interface Slot { id?: string; professionalId: string; professionalName: string; date: string; time: string; office: string; isAvailable: boolean; isBlocked: boolean; createdAt: string; }
...
export default function Agenda() {
  ...
  const formatDate = (d: string) => { const [y, m, day] = d.split('-'); const dias = ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb']; const dt = new Date(Number(y), Number(m)-1, Number(day)); return `${dias[dt.getDay()]}, ${day}/${m}/${y}`; };
  ...
  const filteredItems = useMemo(() => {
    ...
    return allItems.filter((item) => { const d = parseISO(item.date); if (view === 'dia') return isToday(d); if (view === 'semana') return isThisWeek(d); if (view === 'mes') return isWithinInterval(d, { start: startOfMonth(new Date()), end: endOfMonth(new Date()) }); return true; });
  }, [allItems, view]);

  const submit = (e: React.FormEvent) => { e.preventDefault(); addScheduleItem({ ...form, professionalId: currentUser?.role === 'admin' ? form.professionalId : professionalId }); setForm({ beneficiaryId: '', professionalId, date: '', time: '', type: '', notes: '', status: 'agendado' }); };
  const saveStatus = (id: string, status: ScheduleItem['status']) => { updateScheduleItem(id, { status }); setEditingItemId(null); };

  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-semibold text-gray-900">{currentUser?.role === 'consulta' ? 'Minha Agenda' : 'Agenda'}</h1><p className="text-sm text-gray-500">{currentUser?.role === 'admin' ? 'Agenda completa.' : 'Sua agenda profissional.'}</p></div>
      ...
      {activeTab === 'agendamentos' && (<>
        ...
        <div className="flex gap-2 flex-wrap">
          {(['todos','dia','semana','mes'] as const).map((v) => (<button key={v} onClick={() => setView(v)} className={`px-4 py-2 rounded-md text-sm font-medium ${view === v ? 'bg-yellow-400 text-gray-950' : 'bg-white border text-gray-600'}`}>{v === 'todos' ? 'Todos' : v === 'dia' ? 'Hoje' : v === 'semana' ? 'Esta Semana' : 'Este Mês'}</button>))}
        ...
        <div className="rounded-2xl border border-yellow-100 bg-white shadow-sm overflow-hidden">
          {filteredItems.length === 0 ? <p className="p-6 text-center text-gray-500">Nenhum agendamento.</p> : filteredItems.map((item) => {
            const ben = beneficiaries.find((b) => b.id === item.beneficiaryId); const prof = professionals.find((p) => p.id === item.professionalId); const st = item.status || 'agendado'; const isEditing = editingItemId === item.id;
            return (<div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between border-b p-4 gap-3"><div className="flex-1"><p className="font-semibold text-gray-900">{item.date ? new Date(`${item.date}T${item.time || '00:00'}`).toLocaleString() : '-'} - {item.type}</p><p className="text-sm text-gray-500">{ben?.fullName || 'Beneficiário'} | {prof?.name || currentUser?.name} {item.notes ? `| ${item.notes}` : ''}</p><span className={`mt-1 inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${statusColors[st]}`}>{statusLabels[st]}</span></div>
              <div className="flex items-center gap-2 flex-wrap">
                {isEditing ? (<><select defaultValue={st} id={`status-${item.id}`} className="text-xs border rounded-md p-1.5"><option value="agendado">Agendado</option><option value="presente">Presente</option><option value="falta">Falta</option><option value="falta_justificada">Falta Justificada</option><option value="ausencia">Declaração de Ausência</option></select><button onClick={() => { const el = document.getElementById(`status-${item.id}`) as HTMLSelectElement; saveStatus(item.id, el.value as ScheduleItem['status']); }} className="text-green-700 p-1.5 border rounded bg-green-50"><Check size={16}/></button></>) : (<button onClick={() => setEditingItemId(item.id)} className="text-blue-700 p-1.5 border rounded"><Edit2 size={16}/></button>)}
      ...
      {activeTab === 'disponibilidade' && (<div className="space-y-4">
        <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-2xl border border-yellow-100">
          <div className="flex items-center gap-2"><label className="text-sm font-medium">Mês/Ano:</label><input type="month" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="border rounded-md p-2 text-sm" /></div>
          <button onClick={openNewSlot} className="bg-yellow-400 hover:bg-yellow-500 text-gray-950 px-4 py-2 rounded-md flex items-center gap-2 font-semibold text-sm"><Plus size={18} /> Adicionar Horário</button>
          <div className="flex gap-3 text-xs ml-auto"><span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-green-400 inline-block"></span> Disponível</span><span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-red-400 inline-block"></span> Bloqueado</span></div>
        </div>
        {isLoadingSlots ? (<div className="flex justify-center py-12"><Loader2 className="animate-spin text-yellow-500" size={32} /></div>
        ) : Object.keys(grouped).length === 0 ? (<div className="text-center py-12 text-gray-400 bg-white rounded-2xl border border-yellow-100"><Calendar size={48} className="mx-auto mb-3 opacity-50" /><p>Nenhum horário cadastrado.</p></div>
        ) : (<div className="space-y-4">{Object.entries(grouped).sort().map(([date, dateSlots]) => (
          <div key={date} className="border rounded-lg overflow-hidden bg-white">
            ...
            <div className="divide-y">{dateSlots.sort((a, b) => a.time.localeCompare(b.time)).map((slot) => (
      ...
      {isSlotFormOpen && (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[60] p-4"><div className="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">{editingSlot ? 'Editar Horário' : 'Novo Horário'}</h3><button onClick={() => setIsSlotFormOpen(false)} className="text-gray-500"><X size={20} /></button></div>
        <form onSubmit={handleSaveSlot} className="space-y-4">
          ...
          <div className="flex items-center gap-6 py-2"><label className="flex items-center gap-2"><input type="checkbox" checked={slotForm.isAvailable} onChange={(e) => setSlotForm({ ...slotForm, isAvailable: e.target.checked })} className="rounded" /><span className="text-sm text-green-700 font-medium">Disponível</span></label><label className="flex items-center gap-2"><input type="checkbox" checked={slotForm.isBlocked} onChange={(e) => setSlotForm({ ...slotForm, isBlocked: e.target.checked })} className="rounded" /><span className="text-sm text-red-600 font-medium">Bloqueado</span></label></div>
          <div className="flex justify-end gap-2"><button type="button" onClick={() => setIsSlotFormOpen(false)} className="px-4 py-2 border rounded-md text-sm">Cancelar</button><button type="submit" disabled={isSavingSlot} className="px-6 py-2 bg-yellow-400 text-gray-950 font-bold rounded-md text-sm disabled:bg-gray-300">{isSavingSlot ? 'Salvando...' : 'Salvar'}</button></div>
        </form></div></div>)}
    </div>
  );
} 
