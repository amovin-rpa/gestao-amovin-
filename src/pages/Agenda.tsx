          ...
          <div className="bg-white rounded-2xl shadow-sm border border-yellow-100 p-4 mt-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <Calendar size={20}/> Visão Geral do Mês
            ...
            <div className="grid grid-cols-7 gap-1">
              {monthCalendarDays.map((day, idx) => {
                ...
                return (
                  <div key={idx} className={`min-h-[100px] border rounded-md p-1 text-left ${isTodayDay ? 'bg-yellow-50 border-yellow-400' : 'bg-white'}`}>
                    ...
                    <div className="space-y-0.5">
                      ...
                      {dayAppointments.length > 4 && (
                        <div className="text-[10px] text-gray-500 font-semibold">+{dayAppointments.length - 4} mais</div>
                      )}
                    </div>
                  </div>
                );
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium">{editingItem ? 'Novo dia (selecione 1):' : 'Selecione os dias:'}</label>
                  <div className="flex items-center gap-2">
