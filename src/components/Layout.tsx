...
export default function Layout() {
  ...
  const maxStorage = 50; // Mock 50mb

  const navItems = {
    admin: [
      { name: 'Dashboard', icon: Home, path: '/' },
      { name: 'Agenda Geral', icon: Calendar, path: '/agenda' },
      { name: 'Beneficiários (FRB)', icon: Users, path: '/beneficiarios' },
      { name: 'Profissionais', icon: User, path: '/profissionais' },
      { name: 'Voluntários', icon: Briefcase, path: '/voluntarios' },
      { name: 'Financeiro', icon: Activity, path: '/financeiro' },
            ...
            <nav className="mt-5 px-2 space-y-1">
              {currentNav.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`group flex items-center px-2 py-2 text-base font-medium rounded-md ${location.pathname === item.path ? 'bg-blue-100 text-blue-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                >
                  <item.icon className={`mr-4 flex-shrink-0 h-6 w-6 ${location.pathname === item.path ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'}`} />
                  {item.name}
                </Link>
              ))}
            </nav>
          ...
          <div className="flex-1 flex flex-col min-h-0 border-r border-gray-200 bg-white">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <div className="flex items-center flex-shrink-0 px-4">
                <img src={AMOVIN_LOGO_SRC} alt="Amovin" className="h-16 w-auto object-contain" />
              </div>
              <nav className="mt-8 flex-1 px-2 bg-white space-y-1">
                {currentNav.map((item) => (
                  <Link
                    key={item.name}
                    to={item.path}
                    className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${location.pathname === item.path ? 'bg-blue-100 text-blue-900' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                  >
                    <item.icon className={`mr-3 flex-shrink-0 h-6 w-6 ${location.pathname === item.path ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'}`} />
                    {item.name}
                  </Link>
                ))}
              </nav>
            ...
            <div className="flex-shrink-0 flex border-t border-gray-200 p-4 flex-col gap-2">
              <div className="flex items-center text-xs text-gray-500">
                ...
                Nuvem Ativa ({storageMb}MB / {maxStorage}MB)
              </div>
              <p className="text-xs text-gray-400">
                Informações online para quem acessar.
              </p>
              <button
                onClick={logout}
        ...
        <main className="flex-1 relative z-0 overflow-y-auto focus:outline-none">
          <div className="hidden lg:flex items-center justify-between bg-gradient-to-r from-yellow-50 via-white to-amber-50 border-b border-yellow-100 px-8 py-4">
            <div className="flex items-center gap-4">
              <button onClick={() => window.history.back()} className="inline-flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-amber-800"><ArrowLeft size={18} /> Voltar</button>
              <img src={AMOVIN_LOGO_SRC} alt="Amovin" className="h-14 w-auto object-contain" />
            </div>
