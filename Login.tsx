...
export default function Login() {
  ...
  // Load professionals from Firebase directly for login validation
  const [firebaseProfessionals, setFirebaseProfessionals] = useState<Record<string, unknown>[]>([]);
  ...
  useEffect(() => {
    getDocs(collection(db, 'professionals')).then((snapshot) => {
      ...
      setFirebaseProfessionals(items);
      setFbLoaded(true);
    }).catch(() => {
      setFbLoaded(true); // even on error, allow default admin
    });
  }, []);

  // Use Firebase professionals if available, otherwise use store
  const professionals = fbLoaded && firebaseProfessionals.length > 0 ? firebaseProfessionals : storeProfessionals;
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (activeTab === 'admin') {
      ...
      const adminProf = professionals.find(
        (prof) =>
          String(prof.login || '').toLowerCase() === name.trim().toLowerCase() &&
          String(prof.password || '') === password &&
          String(prof.accessRole || 'consulta') === 'admin'
      );

      if (adminProf) {
        login('admin', String(adminProf.name), String(adminProf.specialty || ''), String(adminProf.id));
        return;
      }
    ...
    if (activeTab === 'consulta' || password.trim()) {
      ...
      const professional = professionals.find(
        (prof) =>
          String(prof.login || '').toLowerCase() === name.trim().toLowerCase() &&
          String(prof.password || '') === password &&
          String(prof.accessRole || 'consulta') === activeTab
      );

      if (!professional) {
        setError('Login, senha ou ambiente de acesso invalido.');
        setLoading(false);
        return;
      }

      login(
        String(professional.accessRole || 'consulta') as 'admin' | 'recepcao' | 'consulta',
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-semibold text-gray-950 bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 disabled:opacity-50"
              >
